...
export default function ContractsPage() {
  ...
  const ContractFormDialog = () => {
    const cf = c.contractForm || {};
    return (
    <Dialog open={showForm} onOpenChange={setShowForm}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {dialogMode === 'edit' ? (cf.editTitle || 'Edit Contract') : (cf.newTitle || 'New Contract')}
          </DialogTitle>
          <DialogDescription>
            {dialogMode === 'edit' ? (cf.editDescription || 'Edit contract details') : (cf.newDescription || 'Enter new contract details')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          {/* Contract Type */}
          <div className="space-y-3">
            <Label className="text-base font-bold">{cf.contractType || 'Contract Type'}</Label>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {CONTRACT_TYPES.map((type) => (
                ...
                >
                  ...
                  <p className="text-xs font-medium">{isAr ? type.labelAr : type.labelEn}</p>
                  <Badge variant="outline" className={`text-[10px] mt-1 ${
                    type.category === 'revenue' ? 'text-emerald-600 border-emerald-200' : 'text-rose-600 border-rose-200'
                  }`}>
                    {getContractCategoryLabel(type.category, isAr)}
                  </Badge>
                </button>
              ))}
            </div>
          ...
          {/* Party */}
          <div className="space-y-3">
            <Label className="text-base font-bold">{cf.party || 'Party'}</Label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>{cf.partyType || 'Party Type'}</Label>
                <Select
                  value={formData.partyType}
                  onValueChange={(v: PartyType) => setFormData(prev => ({ ...prev, partyType: v }))}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="company">{isAr ? 'شركة' : (c.company || 'Company')}</SelectItem>
                    <SelectItem value="vendor">{isAr ? 'مورد' : (c.vendor || 'Vendor')}</SelectItem>
                    <SelectItem value="individual">{isAr ? 'فرد' : (c.individual || 'Individual')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label>{cf.selectCompany || 'Select Company'}</Label>
                <Select value={formData.partyId} onValueChange={handlePartyChange}>
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder={isAr ? 'اختر شركة...' : 'Select a company...'} />
                  </SelectTrigger>
                  <SelectContent>
                    {companies.map(c => (
              ...
              <div className="md:col-span-2">
                <Label>{cf.partyName || 'Party Name'}</Label>
                <Input
                  value={formData.partyName}
                  onChange={(e) => setFormData(prev => ({ ...prev, partyName: e.target.value }))}
                  className="mt-1"
                  placeholder={isAr ? 'اسم الشركة أو المورد...' : 'Company or vendor name...'}
                />
              </div>
            </div>
          </div>
          ...
          {/* Linked Residences */}
          <div className="space-y-3">
            <Label className="text-base font-bold">{cf.linkedResidences || 'Linked Residences'}</Label>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {residences.map(res => (
                <label
                  key={res.id}
                  className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all ${
                    formData.linkedResidences.includes(res.id)
                      ? 'border-primary bg-primary/5'
                      : 'border-muted hover:border-muted-foreground/30'
                  }`}
                >
                  <Checkbox
                    checked={formData.linkedResidences.includes(res.id)}
          ...
          {/* Duration */}
          <div className="space-y-3">
            <Label className="text-base font-bold">{cf.contractDuration || 'Contract Duration'}</Label>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label>{cf.startDate || 'Start Date'}</Label>
                <Input
                  type="date"
              ...
              <div>
                <Label>{cf.endDate || 'End Date'}</Label>
                <Input
                  type="date"
              ...
              <div className="flex items-end pb-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setFormData(prev => ({ ...prev, isOpenEnded: !!v }))}
                  />
                  <span className="text-sm">{isAr ? 'عقد مفتوح' : 'Open-ended'}</span>
                </label>
              </div>
            </div>
          </div>
          ...
          {/* Billing */}
          <div className="space-y-3">
            <Label className="text-base font-bold">{cf.billing || 'Billing'}</Label>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label>{cf.billingType || 'Billing Type'}</Label>
                <Select
                  value={formData.billingType}
                  onValueChange={(v: BillingType) => setFormData(prev => ({ ...prev, billingType: v }))}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="per_person_per_day">{isAr ? 'للفرد/اليوم' : 'Per Person/Day'}</SelectItem>
                    <SelectItem value="per_person_per_month">{isAr ? 'للفرد/الشهر' : 'Per Person/Month'}</SelectItem>
                    <SelectItem value="per_room_per_month">{isAr ? 'للغرفة/الشهر' : 'Per Room/Month'}</SelectItem>
                    <SelectItem value="fixed_monthly">{isAr ? 'مبلغ شهري ثابت' : 'Fixed Monthly'}</SelectItem>
                    <SelectItem value="one_time">{isAr ? 'لمرة واحدة' : 'One Time'}</SelectItem>
              ...
              <div>
                <Label>{cf.unit || 'Unit'}</Label>
                <Input
                  value={formData.billingUnit}
                  onChange={(e) => setFormData(prev => ({ ...prev, billingUnit: e.target.value }))}
                  className="mt-1"
                  placeholder={isAr ? 'شخص/شهر' : 'person/month'}
                />
              </div>
              <div>
                <Label>{cf.advancePayment || 'Advance Payment (Optional)'}</Label>
                <Input
                  type="number"
                  value={formData.advancePayment || ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, advancePayment: Number(e.target.value) }))}
                  className="mt-1"
                  placeholder="0"
                />
              </div>
              <div className="md:col-span-2">
                <Label>{cf.paymentTerms || 'Payment Terms (Optional)'}</Label>
                <Input
          ...
          {/* Services */}
          {getContractTypeInfo(formData.contractType).hasServices && (
            <div className="space-y-3">
              <Label className="text-base font-bold">{cf.services || 'Services'}</Label>
              {(formData.services || []).map((service, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-2 p-3 bg-muted rounded-lg">
                  <Input
                    value={service.name}
                    onChange={(e) => {
                    ...
                    placeholder={cf.serviceName || 'Service Name'}
                  />
                  <Input
                    type="number"
                    value={service.rate || ''}
                    onChange={(e) => {
                      const newServices = [...(formData.services || [])];
                      newServices[index] = { ...newServices[index], rate: Number(e.target.value) };
                      setFormData(prev => ({ ...prev, services: newServices }));
                    }}
                    placeholder={cf.serviceRate || 'Rate'}
                  />
                  <Select
                    value={service.frequency}
                    onValueChange={(v) => {
                      const newServices = [...(formData.services || [])];
                      newServices[index] = { ...newServices[index], frequency: v as any };
                      setFormData(prev => ({ ...prev, services: newServices }));
                    }}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      <SelectItem value="monthly">{cf.monthly || 'Monthly'}</SelectItem>
                      <SelectItem value="quarterly">{cf.quarterly || 'Quarterly'}</SelectItem>
                      <SelectItem value="yearly">{cf.yearly || 'Yearly'}</SelectItem>
                      <SelectItem value="one_time">{cf.oneTime || 'One Time'}</SelectItem>
                    </SelectContent>
                  </Select>
                  <Button
                    variant="destructive"
          ...
          {/* Renewal & Alerts */}
          <div className="space-y-3">
            <Label className="text-base font-bold">{cf.renewalAndAlerts || 'Renewal & Alerts'}</Label>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label>{cf.renewalType || 'Renewal Type'}</Label>
                <Select
                  value={formData.renewalType}
                  onValueChange={(v: any) => setFormData(prev => ({ ...prev, renewalType: v }))}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="manual">{cf.manual || 'Manual'}</SelectItem>
                    <SelectItem value="auto_monthly">{cf.autoMonthly || 'Auto Monthly'}</SelectItem>
                    <SelectItem value="auto_quarterly">{cf.autoQuarterly || 'Auto Quarterly'}</SelectItem>
                    <SelectItem value="auto_yearly">{cf.autoYearly || 'Auto Yearly'}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label>{cf.noticePeriod || 'Notice Period (Days)'}</Label>
                <Input
                  type="number"
              ...
              <div className="flex items-end pb-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setFormData(prev => ({ ...prev, autoRenew: !!v }))}
                  />
                  <span className="text-sm">{cf.autoRenew || 'Auto Renew'}</span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <Label>{cf.notes || 'Notes'}</Label>
            <Textarea
              value={formData.notes || ''}
              onChange={(e) => setFormData(prev => ({ ...prev, notes: e.target.value }))}
              placeholder={cf.notesPlaceholder || 'Additional notes...'}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowForm(false)}>{cf.cancel || 'Cancel'}</Button>
          <Button onClick={handleSubmit}>
            {dialogMode === 'edit' ? (cf.save || 'Save Changes') : (cf.create || 'Create Contract')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
