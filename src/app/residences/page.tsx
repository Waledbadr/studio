          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="name" placeholder="e.g., Sunshine Apartments" className="col-span-3" />
              </div>
            </div>
            <DialogFooter>
                                ...
                                <div className="flex justify-between items-center">
                                  <h4 className="font-medium">{floor.name}</h4>
                                   <div className="flex items-center">
                                        <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleAddItem('room', floor.id); }}>
                                            <PlusCircle className="h-4 w-4" />
                                        </Button>
                                    </div>
                                </div>
                                <div className="mt-2 space-y-1">
                                  {floor.rooms.map((room) => (
                                    <div key={room.id} className="flex justify-between items-center text-sm p-2 rounded-md hover:bg-muted">
                                      ...
                                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); handleDeleteItem('room', room.id); }}>
                                        <Trash2 className="h-4 w-4 text-destructive" />
                                      </Button>
                                    </div>
                                  ))}
