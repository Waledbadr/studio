...
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@estatecare/ui/table';
import { Skeleton } from '@estatecare/ui/skeleton';
...
export default function StockMovementReportPage() {
  const { getAllInventoryTransactions, inventoryItems } = useInventory();
  ...
  const availableBuildings = useMemo(() => {
    ...
    return buildings.filter(b => b?.residenceId === filters.residenceId);
  }, [buildings, filters.residenceId]);
  ...
  const availableRooms = useMemo(() => {
    ...
    return rooms.filter(r => r?.floorId === filters.floorId);
  }, [rooms, filters.floorId]);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const filteredTransactions = allTransactions.filter(transaction => {
        ...
        // Filter by building
        if (filters.buildingId && transaction.buildingId !== filters.buildingId) {
          return false;
        }
        
        // Filter by floor
        if (filters.floorId && transaction.floorId !== filters.floorId) {
          return false;
        }
        
        // Filter by room
        if (filters.roomId && transaction.roomId !== filters.roomId) {
          return false;
        }
        
        // Filter by movement type
        if (filters.movementType && transaction.movementType !== filters.movementType) {
          return false;
        }
  ...
  const getMovementTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'ADJUSTMENT_INCREASE': return 'Adjustment Increase';
      case 'ADJUSTMENT_DECREASE': return 'Adjustment Decrease';
  ...
  const getLocationString = (transaction: any) => {
    ...
    if (transaction.roomName) parts.push(transaction.roomName);
    return parts.join(' > ');
  };
                ...
                <TableBody>
                  {transactions.map((transaction, index) => (
                    <TableRow key={index} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <Badge className={`${getMovementTypeColor(transaction.movementType)} border-0 font-medium`}>
                          {getMovementTypeLabel(transaction.movementType)}
                        </Badge>
                      </TableCell>
